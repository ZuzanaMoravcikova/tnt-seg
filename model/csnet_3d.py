# This file is partially adapted from:
# CS2-Net: "cs2net" implementation (c) 2020 ineedzx, MIT License.
# Original repository: https://github.com/iMED-Lab/CS-Net
...
def deconv(in_channels: int, out_channels: int):
...
class ResEncoder3d(nn.Module):
    """Residual encoder block with two 3×3×3 convolutions.

    The block applies two 3D convolutions (each followed by batch
    normalization and ReLU), and adds a 1×1×1 projection of the input as a
    residual connection.
...
class Decoder3d(nn.Module):
    """Decoder block with two 3×3×3 convolutions.
...
class SpatialAttentionBlock3d(nn.Module):
    """Spatial attention via directional cues and non-local weighting.

    The block forms three directional embeddings (query, key, judge)
    using anisotropic kernels to emphasize oriented spatial features,
    computes an affinity matrix in the flattened spatial domain, and
    re-weights features by a learned mixture of the value projection
    and the input (residual).

    Input/Output shape: ``[B, C, Z, Y, X]``.
...
class ChannelAttentionBlock3d(nn.Module):
    """Channel attention via complementary affinity in the channel domain.

    The block computes a complementary channel-wise affinity and uses
    it to re-weight feature responses across channels.
...
class AffinityAttention3d(nn.Module):
    """Fusion of spatial and channel attention with residual addition.

    This block applies spatial attention and channel attention to the
    input tensor and fuses them with a residual connection.
...
class CSNet3D(nn.Module):
    """3D encoder–decoder with bottleneck affinity attention.

    The architecture follows a U-Net–like layout with residual encoder
    blocks, anisotropic pooling/upsampling (Z preserved), and a fusion of
    spatial and channel attention at the deepest feature level. The output
    is passed through a sigmoid to form per-voxel probabilities.

    Parameters
    ----------
    classes: Number of output channels (e.g., 1 for binary segmentation).
    channels: Number of input channels in the volume.
    """
    def __init__(self, classes: int, channels: int) -> None:
